...
class Game:
    def __init__(self, master, player1, player2, Q_learn=None, Q={}, alpha=0.3, gamma=0.9):
        ...
        for i in range(3):
            for j in range(3):
                self.buttons[i][j] = tk.Button(frame, height=3, width=6, text=self.empty_text, command=lambda i=i, j=j: self.callback(self.buttons[i][j]))
    ...
    def reset(self):
        ...
        # Optionally reset random seed for reproducibility
